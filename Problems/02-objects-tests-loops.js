...
function stringIterator (aString, aNumber) {
  ...
  for (var i=0; i<aNumber; i++) {
    returnString = returnString.concat(aString)
  }
  return returnString
}
...
function prettyIterator (aString, aNumber) {
  ...
  let returnString = ''
  for (var i=0; i<aNumber; i++){
    returnString = returnString.concat (aString + '(' + (i+1) + ')\n')
  }
  return returnString
}
...
function computeReign (pm) {
  ...
  const reign = pm.to - pm.from 
  return pm.fullName + '\'s reign was ' + reign + ' years long.'
}
...
let returnString = ''
list.forEach((item) => {
  returnString += (computeReign(item) + '\n')
})
return returnString
}
